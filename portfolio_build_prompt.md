...
- **Mobile App Developer, Daga Meter Taxi PLC (Client Project)** — Sep 2025–Jan 2026 — designed and deployed a production ride-hailing system, launched publicly on Google Play under the brand **Daga Taxi** (passenger + driver apps); built real-time ride tracking using Google Maps, a Socket.io-based driver dispatch connection, and FusedLocationProviderClient with a foreground service for background location updates; Firebase Auth, payment gateway integration, FCM push notifications; owned the full release cycle
